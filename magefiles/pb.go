...
func (Pb) Export(ctx context.Context) error {
	...
	for _, plant := range recordJson.(map[string]interface{})["items"].([]interface{}) {
		...
		for _, iId := range plant.(map[string]interface{})["images"].([]interface{}) {
			if found, ok := returnById(string(iId.(string)), plant.(map[string]interface{})["expand"].(map[string]interface{})["images"].([]interface{})); ok {
				images = append(images, MarshalImage{
					...
					RelativePath: fmt.Sprintf("plants/%s/%s/original.jpg", plant.(map[string]interface{})["id"].(string), iId),
				})
			}
		}
