				...
				<div class="flow-root">
					<div class="-mx-4 flex items-center divide-x divide-gray-200">
						<div class="relative inline-block px-4 text-left">
							...
							<div class:hidden={!affectsOpen} class="absolute right-0 z-10 mt-2 origin-top-right rounded-md bg-white p-4 shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none">
									...
									{#if filteredSymptoms.length}
										{#each filteredSymptoms as symptom, i}
											<div class="flex items-center px-4 mb-4">
												...
												/>
												<label for="filter-symptom-{i}" class="ml-3 whitespace-nowrap text-sm font-medium text-gray-900 cursor-pointer"
													>{symptom.name}
													<span class="rounded bg-gray-200 py-0.5 px-1.5 text-xs font-semibold tabular-nums text-gray-700">{symptom.count}</span></label
												>
											</div>
										{/each}
									{:else}
