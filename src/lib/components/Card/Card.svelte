<script lang="ts">
	...
	import { filters, type FilterItem } from '$components/FilterPanel/filters';

	function toggleFilter(type: string, term: string) {
		...
		<div class="flex-1">
			...
			<p class="px-2.5 pb-2.5 text-sm font-medium space-x-1 space-y-1">
				{#each plant.animals.slice(0, 4) as animal}
					<span
						on:keypress={() => {
							toggleFilter('affects', animal);
						}}
						on:click={() => {
							toggleFilter('affects', animal);
						}}
						class="inline-flex items-center rounded-md bg-{getByAnimal(animal)
							.background} px-2.5 py-1 text-{getByAnimal(animal).foreground} cursor-pointer"
			...
			<p class="px-2.5 pb-2.5 text-sm text-slate-500">
				{#each plant.symptoms.slice(0, 3) as symptom, i}
					<span
						on:keypress={() => {
							toggleFilter('symptoms', symptom.name);
						}}
						on:click={() => {
							toggleFilter('symptoms', symptom.name);
						}}
						title="View other plants with this symptom."
						class="border-b border-dotted border-b-green-600 text-green-600 hover:text-green-500 cursor-pointer"
						>{symptom.name}</span
