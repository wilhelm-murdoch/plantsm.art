<script lang="ts">
	import { PullRequestEvent } from '$components';
...
<li class="relative py-2">
	<div class="flex items-center mb-1 pb-5">
		...
		/>
		<div
			class="absolute rounded-full p-1 text-{eventIcons.PullRequestEvent[0]}-800 bg-{eventIcons
				.PullRequestEvent[0]}-100"
			aria-hidden="true"
		>
			<svg
				class="w-6 h-6"
				...
				viewBox="0 0 24 24"
				stroke="currentColor"
				xmlns="http://www.w3.org/2000/svg">{@html eventIcons.PullRequestEvent[1]}</svg
			>
		</div>
		<h3 class="text-sm font-bold text-slate-800 pl-12">
			{event.payload.action.charAt(0).toUpperCase() + event.payload.action.slice(1)} the pull request
			<a
