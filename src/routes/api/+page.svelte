		...
		>
			...
			<div class="prose prose-lg prose-slate px-4 mx-auto text-gray-500">
				...
				<p class="text-xl leading-8 text-gray-500 text-justify">
					...
					it into a minimal JSON-based static database. The entirety of the site is powered by this single
					file and you're more than welcome to use it for your own purposes.
				</p>

				<p>
					We also expose several other JSON files which can be used as lookup tables in combination
					with <code>plants.json</code> to help sort through the data.
				</p>
				...
				<ul class="text-gray-400">
					<li>
						...
						<p>
							Our primary database. This includes a breakdown of all currently-supported plants
							along with their common names, symptoms, images and affected animal types.
						</p>
					...
					<li>
						...
						<p>
							A distinct listing of all known symptoms with an associated listing of any related
							plant records.
						</p>
						<p>This is an array of objects with the following structure:</p>
					...
					<li>
						...
						<p>
							A distinct listing of all supported animal types with an associated listing of any
							...
							{#each getAllAnimals() as animal}
								<li>
									<span
										...
										>{getByAnimal(animal).emoji}</span
									> <a href="/{animal}.json"><code>{animal}.json</code></a>
								</li>
							{/each}
						</ul>
					</li>
				...
				<h3>Usage</h3>
				<p>
					These are known as "dumb" API endpoints. They are static flat files that are generated at
					"build time" before we deploy changes to production. They, like this site, live behind a
					CDN which is managed by Cloudflare. We do not impose rate-limits or any other restrictions
					with regards to access.
				</p>
				<p>You may download these files locally without attribution and use them as you wish.</p>
				<p>
					If you wish to use the associated images, be sure to abide by the associated <a
