<script lang="ts">
	import { ClassificationCard, Warning, DetailsCard } from '../../../components';

	export let data: any;
</script>

<div class="fade-in-animation">
	<div class="mb-4">
		{#if data.plant.symptoms.includes('death')}
			<h1
				class="unstyled bg-black p-2.5 text-4xl font-extralight text-white rounded-tr-md rounded-tl-md"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					x="0px"
					y="0px"
					viewBox="0 0 512 512"
					class="svg-icon inline-block outline-none fill-current w-5 h-5 -translate-y-0.5"
					focusable="false"
					data-testid="svg-icon"
					><path
						d="M416 400V464C416 490.5 394.5 512 368 512H320V464C320 455.2 312.8 448 304 448C295.2 448 288 455.2 288 464V512H224V464C224 455.2 216.8 448 208 448C199.2 448 192 455.2 192 464V512H144C117.5 512 96 490.5 96 464V400C96 399.6 96 399.3 96.01 398.9C37.48 357.8 0 294.7 0 224C0 100.3 114.6 0 256 0C397.4 0 512 100.3 512 224C512 294.7 474.5 357.8 415.1 398.9C415.1 399.3 416 399.6 416 400V400zM160 192C124.7 192 96 220.7 96 256C96 291.3 124.7 320 160 320C195.3 320 224 291.3 224 256C224 220.7 195.3 192 160 192zM352 320C387.3 320 416 291.3 416 256C416 220.7 387.3 192 352 192C316.7 192 288 220.7 288 256C288 291.3 316.7 320 352 320z"
					/></svg
				>
				{data.plant.name}
			</h1>
			<Warning />
		{:else}
			<h1
				class="unstyled bg-gradient-to-tr from-green-700 to-emerald-900 p-2.5 text-4xl font-extralight text-white rounded-md"
			>
				{data.plant.name}
			</h1>
		{/if}
	</div>

	<div class="mx-auto grid max-w-lg lg:max-w-none lg:grid-cols-2 gap-4">
		<DetailsCard common={data.plant.common} symptoms={data.plant.symptoms} plant={data.plant} />

		<ClassificationCard
			animals={data.plant.animals}
			classification={data.plant.classification}
			wikipedia_url={data.plant.wikipedia_url}
		/>
	</div>

	<div class="text-center text-sm text-slate-300 mt-4">
		Last Updated: {data.plant.date_last_updated}
	</div>
</div>

<style>
	.fade-in-animation {
		-webkit-animation: fade-in-animation 0.5s cubic-bezier(0.39, 0.575, 0.565, 1) both;
		animation: fade-in-animation 0.5s cubic-bezier(0.39, 0.575, 0.565, 1) both;
	}

	@-webkit-keyframes fade-in-animation {
		0% {
			opacity: 0;
		}
		100% {
			opacity: 1;
		}
	}
	@keyframes fade-in-animation {
		0% {
			opacity: 0;
		}
		100% {
			opacity: 1;
		}
	}
</style>
