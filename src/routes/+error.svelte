<div class="relative bg-gray-50 pb-5 mb-8 sm:px-6 sm:pt-4 lg:px-8 lg:pt-4 lg:pb-4 border-y">
	<main class="mx-auto text-center flex w-full max-w-7xl flex-grow flex-col px-4 sm:px-6 lg:px-8">
		<div class="my-auto flex-shrink-0 py-16 sm:py-32">
			<p class="text-base font-semibold text-green-600">404</p>
			<h1 class="mt-2 text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl">
				Page not found
			</h1>
			<p class="mt-2 text-base text-gray-500">Looks like this page went on its ☕️ break.</p>
			<div class="mt-6">
				<a href="/" class="text-base font-medium text-green-600 hover:text-green-500">
					Go back home
					<span aria-hidden="true"> &rarr;</span>
				</a>
			</div>
		</div>
	</main>
</div>
