<script lang="ts">
	...
	function addFilter(type: string, term: string) {
	...
	function filterExists(filter: FilterItem): boolean {
	...
	function handleSearchInput(e: KeyboardEvent): void {
		if (e.key == 'Enter') {
			let searchInputElement = <HTMLInputElement>document.getElementById('search');
			...
			<div class="relative inline-block text-left w-1/2">
				<div class="relative mt-1 flex items-center">
					<div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
						<svg
							class="-ml-1 mr-3 h-5 w-5 text-gray-400"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 20 20"
					...
					{#if $filters.length}
						{#each $filters as filter, i}
							...
							{:else if filter.type == 'symptoms'}
								{#if filter.term == 'Death'}
									<span
										in:fade
										class="m-1 inline-flex items-center rounded-md shadow-sm text-white py-1.5 pl-3 pr-2 text-sm font-medium bg-black"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											x="0px"
										...
										<button
											type="button"
											class="ml-1 inline-flex h-4 w-4 flex-shrink-0 rounded-full p-1 text-white bg-slate-600 hover:bg-slate-700"
											on:click={() => removeFilter(filter)}
										>
											<svg class="h-2 w-2" stroke="currentColor" fill="none" viewBox="0 0 8 8">
												<path stroke-linecap="round" stroke-width="1.5" d="M1 1l6 6m0-6L1 7" />
							...
							{:else}
								<span
									in:fade
									class="m-1 inline-flex items-center rounded-md shadow-sm bg-white py-1.5 pl-3 pr-2 text-sm font-medium text-gray-900"
								>
									<span>{filter.term}</span>
									<button
										type="button"
